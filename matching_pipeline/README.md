...
Telemetry now lives in the independent top-level [`telemetry/`](../telemetry/) package rather than the matching-pipeline shared layer. Its Compose service, protocol, scheduling, limits, security controls, migrations, and operational commands are documented in [`telemetry/README.md`](../telemetry/README.md).

**Export scope:** enabling telemetry inventories the complete historical `match_score` graph without a date predicate. The seven-day window applies only to recent summary counts; it does not restrict which historical rows a receiver may request.
...
```bash
python -m compileall -q matching_pipeline telemetry scripts/run_pipeline_scheduler.py
python -m pytest -q tests/matching_pipeline tests/telemetry
python -m pytest -q
```
