...
`DINOV3_MODEL_ID` is the sole deployed checkpoint selector; abbreviated size-key aliases are not supported. Use the full Hugging Face model ID so model loading, embedding-cache validation, and candidate identity agree. Before downloading/loading DINOv3 weights, blocking uses Hugging Face safetensors parameter metadata and the selected runtime dtype to reject obvious GPU-fit failures. Successful metadata responses are cached once per model/revision under `${CACHE_DIR}/model_metadata/huggingface`; the resolved commit is reused for the model and processor. Metadata/API failure is fail-open, while insufficient GPU memory remains fatal. The check logs free, total, allocated, and reserved GPU memory and adds no inference-workspace estimate.
...
| Intentionally refresh `main` for an unchanged `DINOV3_MODEL_ID` | Remove `${CACHE_DIR}/model_metadata/huggingface`, then lost `embeddings.npz`; remove candidates only if blocking cannot run immediately afterward |
